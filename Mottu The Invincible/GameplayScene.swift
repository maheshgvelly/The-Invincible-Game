...
class GameplayScene: SKScene, SKPhysicsContactDelegate{
    ...
    private var cameraDistanceBetweenCreatingNewClouds = CGFloat()
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        if firstBody.node?.name == "Player" && secondBody.node?.name == "Life" {
            ...
            NSTimer.scheduledTimerWithTimeInterval(NSTimeInterval(2), target: self, selector: #selector(GameplayScene.playerDied), userInfo: nil, repeats: false)
        }
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if nodeAtPoint(location).name == "Quit" {
                ...
                self.view?.presentScene(scene!, transition: SKTransition.doorsCloseVerticalWithDuration(2))
            }
        }
    ...
    func initializeVariables() {
        ...
        cameraDistanceBetweenCreatingNewClouds = (mainCamera?.position.y)! - 400
    ...
    func createNewClouds(){
        ...
        if cameraDistanceBetweenCreatingNewClouds > mainCamera?.position.y {
            ...
            checkForChildOutOffScreen()
            
        }
        
    }
    
    func checkForChildOutOffScreen() {
